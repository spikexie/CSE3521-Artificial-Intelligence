...
def Logisitc_Regression(X, Y, learningRate=0.01, maxIter=100):
    ...
    dim = w.reshape(D_plus_1, 1)
    for t in range(maxIter):
        tho = sigmoid(np.matmul(w.transpose(), X).reshape(N, 1))
        loss = np.matmul(X, (Y - tho)) / (-N)
        w -= np.dot(learningRate, loss)
        #print(w)
...
def Perceptron(X, Y, learningRate=0.01, maxIter=100):
    ...
    for t in range(maxIter):
        ...
        Y = Y[permutation, :]
        for n in range(N):
            Y_hat = np.sign(np.matmul(X[:,n].transpose(), w))
            if (Y_hat != Y[n]).any():
                pro = (Y[n]*X[:,n]).reshape(D_plus_1,1)
                w += np.dot(learningRate, pro)
