    ...
    def train(self, training_sentences, training_labels):
        ...
        for i in range(0, max.shape[1]):
            ...
            for j in range(0, max.shape[0]):
                ...
                if max[j][i] == 0 and training_labels[j] == -1:
                    Nnega += 1.0
            self.conditional[0][i] = posi / positive
            self.conditional[3][i] = Nposi / positive
            self.conditional[1][i] = neu / neutral
            self.conditional[4][i] = Nneu / neutral
            self.conditional[2][i] = nega / negative
            self.conditional[5][i] = Nnega / negative
